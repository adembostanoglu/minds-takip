// V1.17.7 — Yalnızca yönetici için Sosyal Medya Takip modülü + firma sıra numarası + KDV özeti.
(function bootSocialMediaTrackingV170(){
  ...
  function installStyles(){
    ...
    s.textContent=`
      ...
      .smt-kpis-v170{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:12px;margin-bottom:16px}.smt-kpi-v170{border:1px solid #29343a;border-radius:14px;background:#10171b;padding:16px;min-height:106px;box-sizing:border-box}.smt-kpi-v170 small{display:block;color:#8c989f;font-size:10px;text-transform:uppercase;letter-spacing:.35px;margin-bottom:9px}.smt-kpi-v170 b{display:block;color:#f0f3f4;font-size:22px;line-height:1.15;letter-spacing:-.4px}.smt-kpi-v170 span{display:block;color:#7f8b92;font-size:10px;margin-top:8px}.smt-kpi-v170.cash{border-color:#28533c}.smt-kpi-v170.cash b{color:#91d47a}.smt-kpi-v170.invoice{border-color:#294a61}.smt-kpi-v170.invoice b{color:#8bcdf3}.smt-kpi-v170.vat{border-color:#5b4928;background:#18160f}.smt-kpi-v170.vat b{color:#e8c46f}.smt-kpi-v170.total{border-color:#625d22;background:#191a0e}.smt-kpi-v170.total b{color:#ece52c}.smt-kpi-v170.package b{color:#e8edef}
      ...
      .smt-table-wrap-v170{overflow:auto}.smt-table-v170{width:100%;border-collapse:collapse;min-width:1080px}.smt-table-v170 th,.smt-table-v170 td{padding:13px 14px;border-bottom:1px solid #222c31;text-align:left;white-space:nowrap;font-size:12px}.smt-table-v170 th{color:#8d999f;font-size:10px;text-transform:uppercase;letter-spacing:.25px;background:#12191d}.smt-table-v170 tr:last-child td{border-bottom:0}.smt-no-v175{width:48px;min-width:48px;max-width:48px;text-align:center!important;color:#ece52c!important;font-weight:900;font-variant-numeric:tabular-nums;letter-spacing:.35px}.smt-firm-v170{display:flex;align-items:center;gap:10px}.smt-firm-v170 .firm-logo{width:34px;height:34px;min-width:34px}.smt-price-v170{font-weight:800;color:#eef2f3}.smt-package-v170{font-weight:800;color:#ece52c}.smt-note-v170{max-width:210px;overflow:hidden;text-overflow:ellipsis;color:#b7c0c4}.smt-pay-v170{display:inline-flex;border-radius:7px;padding:5px 8px;font-size:10px;font-weight:800}.smt-pay-v170.elden{background:#113522;color:#91d47a}.smt-pay-v170.fatura{background:#122b3b;color:#8bcdf3}.smt-pay-v170.none{background:#262d31;color:#9ba5aa}.smt-footer-v170{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:13px 18px;border-top:1px solid #263036;color:#829097;font-size:11px}.smt-footer-v170 b{color:#e8edef}
      ...
      @media(max-width:1420px){.smt-kpis-v170{grid-template-columns:repeat(4,1fr)}}@media(max-width:900px){.smt-kpis-v170{grid-template-columns:repeat(2,1fr)}}@media(max-width:760px){.smt-panel-head-v170{align-items:stretch;flex-direction:column}.smt-search-v170{width:100%}.smt-detail-form-v170{grid-template-columns:1fr}.smt-field-v170.full{grid-column:auto}}
    `;
  ...
  function render(){
    ...
    const orderNo=new Map(allFirms.map((f,i)=>[f.id,i+1]));
    const firms=allFirms.filter(f=>!searchTerm||String(f.name||'').toLocaleLowerCase('tr-TR').includes(searchTerm));
    ...
    const vatAmount=totals.invoice*(VAT_RATE/(1+VAT_RATE));
    host.innerHTML=`
      ...
      <div class="smt-kpis-v170">
        ...
        <div class="smt-kpi-v170 invoice"><small>Fatura Kesilen</small><b>${money(totals.invoice)}</b><span>KDV dahil fatura toplamı</span></div>
        <div class="smt-kpi-v170 vat"><small>Çıkan KDV (%20)</small><b>${money(vatAmount)}</b><span>KDV dahil tutarın içindeki vergi</span></div>
      ...
      <div class="smt-panel-v170">
        ...
        <div class="smt-table-wrap-v170"><table class="smt-table-v170"><thead><tr><th class="smt-no-v175">No</th><th>Firma Adı</th><th>Aylık Fiyat</th><th>Paket</th><th>Paket İçeriği</th><th>Başlangıç Tarihi</th><th>Ödeme Şekli</th><th>Durum</th><th>Not</th><th>İşlem</th></tr></thead><tbody>${firms.map(f=>{
          const r=rowFor(f.id),pkg=currentPackage(f),pay=r?.payment_type||'',start=r?.start_date||String(f.created_at||'').slice(0,10),has=!!r,no=orderNo.get(f.id)||1;
          return `<tr><td class="smt-no-v175">${String(no).padStart(2,'0')}</td><td><div class="smt-firm-v170">${typeof firmLogo==='function'?firmLogo(f):''}<b>${esc(f.name)}</b></div></td><td class="smt-price-v170">${has?money(r.monthly_price):'—'}</td><td class="smt-package-v170">${pkg.post}+${pkg.video}</td><td>${pkg.post} Post + ${pkg.video} Video</td><td>${formatDateSafe(start)}</td><td><span class="smt-pay-v170 ${pay||'none'}">${paymentLabel(pay)}</span></td><td><span class="badge ${f.active?'green':'orange'}">${f.active?'Aktif':'Pasif'}</span></td><td><div class="smt-note-v170" title="${esc(r?.note||'')}">${esc(r?.note||'—')}</div></td><td><button class="small-primary" data-smt-detail="${f.id}">Detay</button></td></tr>`;
        }).join('')||'<tr><td colspan="10"><div class="smt-empty-v170">Bu ay için firma bulunamadı.</div></td></tr>'}</tbody></table></div>
