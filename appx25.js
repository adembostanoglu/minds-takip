// V1.13.8 — previous-month carryover backlog, separate from the new month's package quota.
(function bootCarryoverV138(){
  if(typeof sb==='undefined' || typeof state==='undefined' || typeof renderAll!=='function'){
    setTimeout(bootCarryoverV138,120); return;
  }
  if(window.__mindsCarryoverV138) return;
  window.__mindsCarryoverV138=true;
  let lastRows=[];
  ...
  function ensurePanel(){
    ...
    if(!panel){ panel=document.createElement('section'); panel.id='carryoverPanelV138'; panel.className='panel'; panel.style.marginTop='12px'; const grid=dash.querySelector('.content-grid'); grid?.insertAdjacentElement('afterend',panel); }
    return panel;
  }
  async function refreshCarryover(){
    ...
    panel.innerHTML=`<div class="panel-head"><div><h3>Geçen Aydan Kalanlar</h3><p>${monthLabel(source)} ayından ${monthLabel(selectedMonth)} ayına devreden eksik içerikler</p></div></div><div class="empty compact-empty">Kontrol ediliyor...</div>`;
    const {data,error}=await sb.rpc('carryover_preview',{p_source_month:source});
    if(error){ panel.innerHTML=`<div class="panel-head"><div><h3>Geçen Aydan Kalanlar</h3><p>${monthLabel(source)} eksikleri</p></div></div><div class="empty compact-empty">Liste alınamadı.</div>`; console.warn('carryover_preview',error); return; }
    lastRows=data||[];
    const canComplete=selectedMonth===currentMonth();
    panel.innerHTML=`<div class="panel-head"><div><h3>Geçen Aydan Kalanlar</h3><p>${monthLabel(source)} ayında eksik kalan işler yeni ay paketine karışmadan burada takip edilir.</p></div><span class="badge ${lastRows.length?'orange':'green'}">${lastRows.length?`${lastRows.length} eksik kalem`:'✓ Eksik yok'}</span></div>
      ${lastRows.length?`<div class="table-wrap"><table><thead><tr><th>Firma</th><th>İçerik</th><th>Paket</th><th>Önceki Ay Hazır</th><th>Devirde Tamamlanan</th><th>Kalan</th><th>Sorumlu</th><th>İşlem</th></tr></thead><tbody>${lastRows.map((r,i)=>`<tr><td><b>${esc(r.firm_name)}</b></td><td><span class="badge ${r.type==='post'?'blue':'yellow'}">${r.type==='post'?'Post':'Video'}</span></td><td>${r.package_qty}</td><td>${r.prepared_qty}</td><td>${r.carryover_done}</td><td><b>${r.remaining_qty}</b></td><td>${(r.responsible_names||[]).map(esc).join(', ')||'—'}</td><td>${canComplete?`<button class="small-primary" data-carryover-complete="${i}">Tamamlandı</button>`:'<span class="muted">Sadece güncel ayda kapatılır</span>'}</td></tr>`).join('')}</tbody></table></div>`:`<div class="empty compact-empty">${monthLabel(source)} ayından devreden eksik içerik yok.</div>`}`;
  }
  ...
  const prev=renderAll; renderAll=function(){ prev(); setTimeout(refreshCarryover,50); };
