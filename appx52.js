// V1.19.1 — Mesai personel sabit sıralaması ve sıra numaraları; açılır detay satırlarını sıralama dışında tutar.
(function bootAttendanceStaffOrderV191(){
  if(window.__mindsAttendanceStaffOrderV191)return;
  window.__mindsAttendanceStaffOrderV191=true;
  ...
  function patchPayrollTable(){
    ...
    if(!tbody)return;
    const rows=[...tbody.querySelectorAll(':scope > tr')].filter(r=>r.querySelector('td')&&!r.classList.contains('att-person-expand-row-v191'));
