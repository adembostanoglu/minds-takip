...
(function bootAttendanceInlineDetailsV223(){
  ...
  function buildFocus(pid,token){
    ...
    if(!row||!source||!anchor)return;
    if(sel&&sel.value!==pid)return;
  ...
  function openPerson(pid){
    ...
    // Personel seçimini ana Mesai motorunun kendi Detay akışına bırak.
    // Böylece selectedPersonId ve render sırası tek kaynaktan yönetilir.
    const row=rowForPid(pid);
    ...
    [700,1100].forEach(ms=>setTimeout(()=>{
      if(token===requestToken&&expandedPid===pid&&!document.getElementById('attPersonFocusV195')){
        buildFocus(pid,token);
      }
    },ms));
  }
