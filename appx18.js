// V1.13 — automatic monthly performance scoring + Employee of the Month badge
(function bootPerformanceV113(){
  if(typeof sb==='undefined' || typeof el!=='function' || typeof setView!=='function' || !profile){
    setTimeout(bootPerformanceV113,120);
    return;
  }
  if(window.__mindsPerformanceV113) return;
  window.__mindsPerformanceV113=true;

  let selectedDetailId=null;
  let refreshTimer=null;
  let badgeKey='';
  const roleLabels={tasarim:'Tasarım / Post',video:'Video',sosyal_medya:'Sosyal Medya'};
  ...
  function ensureAdminUI(){
    ...
    if(!nav){
      ...
      nav.onclick=()=>{ setView('performance'); el('pageTitle').textContent='Performans'; if(el('pageSub'))el('pageSub').textContent='Aylık otomatik performans puanları ve Ayın Personeli.'; refreshPerformance(true); };
    }
    ...
    if(!section){
      ...
      section.innerHTML=`<div class="section-actions"><div><h2>Performans</h2><p>Puanlar sistem tarafından otomatik hesaplanır; manuel puan yoktur.</p></div></div><div id="performanceBodyV113"></div>`;
  ...
  function roleDetailHtml(details){
    ...
    return `<div class="performance-role-grid-v113">${keys.map(k=>{const r=roles[k]||{},ex=num(r.expected),dn=num(r.done),tm=num(r.timely);return `<div class="performance-role-v113"><b>${esc(roleLabels[k]||k)}</b><span>Beklenen: ${ex}</span><span>Tamamlanan: ${dn} · %${pct(dn,ex)}</span><span>Zamanında: ${tm} · %${pct(tm,ex)}</span></div>`}).join('')}</div>`;
  }
  ...
  function renderPerformance(rows,finalized){
    ...
    body.innerHTML=`
      <div class="info-banner"><b>Performans kuralı:</b> Yeni ayın postları ayın 1'inde “Hazır/Onaylandı” olarak sisteme girilir. Videolar ve o aya ait paylaşımlar ay sonuna kadar tamamlanır.${onboarding?' <b>Ağustos 2026 ilk kurulum ayı olduğu için post zamanlama son tarihi yalnızca bu ay 31 Ağustos olarak uygulanır.</b>':''}</div>
      <div class="performance-rules-v113"><div class="performance-rule-v113"><small>Ana Görev</small><b>60</b><span>Tasarım, video veya sosyal medya sorumluluğunun tamamlanma oranı.</span></div><div class="performance-rule-v113"><small>Zamanlama</small><b>20</b><span>Post: ayın 1'i. Video ve paylaşım: ayın son günü.</span></div><div class="performance-rule-v113"><small>Ekstra Katkı</small><b>10</b><span>5 ekstra işte maksimum puana ulaşır; sınırsız puan üretmez.</span></div><div class="performance-rule-v113"><small>Çekim Katkısı</small><b>10</b><span>4 sorumlu çekimde maksimum puana ulaşır.</span></div></div>
      <div class="performance-top-v113"><div class="performance-leader-v113"><div class="crown">🏆</div><p>${leaderLabel} · ${monthName(selectedMonth)}</p><h3>${leader?esc(leader.full_name):'Henüz sonuç yok'}</h3><strong>${leader?num(leader.total_score).toFixed(1):'—'}</strong><p>${finalized?(awardReady?'Ödül sahibi kesinleşti.':'Minimum 60 puan şartı sağlanmadığı için ödül yok.'):'Ay kapanana kadar sıralama değişebilir.'}</p></div><div class="performance-status-v113"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px"><h4>${monthName(selectedMonth)} Puan Durumu</h4>${status}</div><p>Puanlar tamamen sistem verisinden hesaplanır. Yönetici dahil hiçbir kullanıcı manuel puan ekleyemez. Bir personelin yapmadığı rol, ana görev ortalamasına dahil edilmez. Sosyal medya uzmanı yalnızca paylaşım sorumluluğu bulunan firmalardaki hazır içeriklerden değerlendirilir.</p></div></div>
      <div class="panel"><div class="table-wrap"><table class="performance-table-v113"><thead><tr><th>Sıra</th><th>Personel</th><th>Ana Görev /60</th><th>Zamanlama /20</th><th>Ekstra /10</th><th>Çekim /10</th><th>Toplam</th></tr></thead><tbody>${rows.map(r=>`<tr data-perf-person="${r.person_id}"><td><span class="rank">${r.rank?('#'+r.rank):'—'}</span></td><td><b>${esc(r.full_name)}</b>${!r.eligible?'<div class="score-muted">Puanlanabilir ana görev yok</div>':''}</td><td>${num(r.core_score).toFixed(1)}</td><td>${num(r.timing_score).toFixed(1)}</td><td>${num(r.extra_score).toFixed(1)}</td><td>${num(r.shoot_score).toFixed(1)}</td><td><span class="score-total">${num(r.total_score).toFixed(1)}</span></td></tr>`).join('')||'<tr><td colspan="7" class="empty">Personel puanı bulunamadı.</td></tr>'}</tbody></table></div></div>
    ...
    body.querySelectorAll('[data-perf-person]').forEach(tr=>tr.addEventListener('click',()=>{
      ...
      d.innerHTML=`<div class="performance-detail-v113"><h4>${esc(r.full_name)} · Puan Detayı</h4><p>Ana görev puanı, kişinin o ayki sorumluluk türlerine göre normalize edilir.</p>${roleDetailHtml(r.details)}<div class="performance-role-grid-v113" style="margin-top:10px"><div class="performance-role-v113"><b>Ekstra İş</b><span>${num(r.details?.extra_quantity)} adet</span><span>Puan: ${num(r.extra_score).toFixed(1)} / 10</span></div><div class="performance-role-v113"><b>Çekim</b><span>${num(r.details?.shoot_count)} çekim · ${num(r.details?.shoot_video_count)} video içeriği</span><span>Puan: ${num(r.shoot_score).toFixed(1)} / 10</span></div><div class="performance-role-v113"><b>Toplam</b><span>Ana görev ${num(r.core_score).toFixed(1)} + zaman ${num(r.timing_score).toFixed(1)}</span><span><strong>${num(r.total_score).toFixed(1)} / 100</strong></span></div></div></div>`;
  ...
  function updateVersionLabel(){
    document.querySelectorAll('#settings .settings-list span').forEach(s=>{ if(/^Mind's Takip V/i.test(s.textContent||'')) s.textContent="Mind's Takip V1.13"; });
  }
